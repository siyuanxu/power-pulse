...
- 菜单栏实时读数：使用随系统深浅色自动切换的单色电池图标，电量数字直接写在图标内，旁边显示实时功率；充电时图标带闪电。单击后可在下拉菜单顶部查看完整百分比。接电显示适配器→Mac 输入，电池供电显示电脑实时用电。
...
构建脚本会重新登记小组件扩展，并结束仍占用旧构建二进制的小组件进程；桌面上已有的小组件会由 macOS 自动重新生成时间线。
